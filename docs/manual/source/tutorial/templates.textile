...
Signature = {
    ...
    <span class="highlighted">:templates => {
		...
		<p>Which then further allows the use of the Arrow::Applet#load_template
...
<pre>def_action :templated do |txn, *args|
    self.log.debug "In the 'templated' action of the %s applet." %
        self.signature.name

    <span class="highlighted">templ = self.load_template( :templated )
    templ.txn = txn
...
&lt;!--
  ...
  &lt;body&gt;
	...
	templates).&lt;p&gt;
		...
		&lt;!-- Runcount: <span class="highlighted">[?call applet.run_count ?]</span> --&gt;
...
<span class="highlighted">&lt;?if txn.session? ?&gt;</span>
  ...
  <span class="highlighted">&lt;?if txn.session[:user] ?&gt;</span>
	...
	txn.session[:user]?&gt;</span>', you can also &lt;a
		href="<span class="highlighted">[?call txn.app_root?]</span>/<span class="highlighted">[?call txn.applet_path.split(%r{/})[0]
